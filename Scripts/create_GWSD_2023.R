...
#### FOR 2023 AND 2024- ADD DISTINCT TO IND_CONV
...
get_gwsd <- function(current_month) {
  ...
  #data_in <-"~/Github/dewormr/Data/10. Databases (October 2023)"
  
  data_out <- "~/Github/dewormr/Dataout"
  ind_conv <- read_xlsx("~/Github/dewormr/Data/indicator_conversion.xlsx") %>% distinct()
  ...
  df <- df_rough %>% 
    ...
    mutate(month_num = as.integer(factor(month, levels = month.name)),
           ...
           #month_cur = as.integer(factor("October", levels=month.name)),
